  ...
  return function(req, res, next) {
    ...
    res.auth = {
      remember: function(user) {
        ...
        digest = user['token_digest'] = utils.digest(token);
        res.auth.token = token;
        res.json = decorateJSON(res.json, token);
        return db.save(user.id, user);
      },
      clear: function(user) {
        res.json = decorateJSON(res.json, null);

        var result = true;
        if (user) { // expire token for good measure
          user['token_digest'] = 'expired';
          result = db.save(user.id, user);
        }
        return result;
      }
    };
